...
export class GymsearchComponent implements OnInit {
   ...
   ngOnInit() {
      ...
      this.getCurrentLocation().then((coordinates: GeolocationCoordinates) => {
         ...
         const url = `https://us-central1-sweatsession.cloudfunctions.net/proxyRequest?latitude=${this.currLatitude}&longitude=${this.currLongitude}&radius=${this.maxDistance*1000}&key=${this.MAPS_API_KEY}`;
         // const url = `localhost:4200/proxyRequest?latitude=${this.currLatitude}&longitude=${this.currLongitude}&radius=${this.maxDistance}&key=${this.MAPS_API_KEY}`;
         fetch(url)
    ...
    .catch((error) => {
      ...
      // const proxyRequestFunction = firebase.functions().httpsCallable('proxyRequest');
      // const functions = getFunctions();
      // const proxyRequest = httpsCallable(functions, 'proxyRequest');
      // proxyRequest()
   }
